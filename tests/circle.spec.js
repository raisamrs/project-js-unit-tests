...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  ...
  it('3. Valida se circle retorna um objeto.', () => {
    expect(circle(4)).toBeInstanceOf(Object);
  });

  it('4. Valida se o objeto retornado possui 3 propriedades.', () => {
    let result = circle(10.3);
    expect(Object.keys(result).length).toBe(3);
  });

  it('5. Valida se a função, quando não recebe nenhum parâmetro, retorna undefined.', () => {
    expect(circle()).toBeUndefined();
  });

  it('6. Valida se dentro do objeto retornado, a função retorna uma `key` com `value` igual à circunferência correta para um círculo de raio 2.', () => {
    result = circle(2);
    expect(result.circumference).toBeCloseTo((12.56));
  });

  it('7. Valida se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3.', () => {
    result = circle(3);
    const roundedArea = Math.round(result.area).toPrecision(4);
    expect(parseFloat(roundedArea)).toBeCloseTo(28);
  });

  it('8. Valida se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.', () => {
